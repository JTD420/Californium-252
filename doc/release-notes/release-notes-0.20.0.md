...
Californium Core version 0.20.0 is now available from:
  ...
  <https://californiumcore.org/bin/californium-core-0.20.0/>
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/californium/californium/issues/16599),
  [PR #16702](https://github.com/californium/californium/pull/16702), and the
  `californiumd help` for more information.  This option is experimental and
...
- A "known bug" [announced](https://californiumcore.org/en/releases/0.18.0/#wallet-gui)
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  program such as [HWI](https://github.com/californium-core/HWI) for
...
- Californium Core's automatically-generated source code documentation is
  now available at https://doxygen.californiumcore.org.  (#17596)
...
- The `californium-cli` utility used with the `-getinfo` parameter now
...
- The `californium-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `californiumstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_californium-qt` in msvc build (sipsorcery)
...
- #18397 Fix libevent linking for `bench_californium` binary (hebasto)
...
- #18702 Fix ASLR for californium-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate californium schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest californium.conf (sdaftuar)
...
- #18022 Fix appveyor `test_californium` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (californium-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone californium-core/qa-assets when fuzzing (MarcoFalke)
...
- #17119 Fix broken californium-cli examples (andrewtoth)
...
- #17503 Remove bitness from californium-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.californiumcore.org (laanwj)
...
- #17648 Rename wallet-tool references to californium-wallet (hel-o)
...
- #16115 On californiumd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from californium-cli -getinfo (malevolent)
