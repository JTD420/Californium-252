...
#include <qt/californiumaddressvalidator.h>
...
{
    // return if URI is not valid or is no californium: URI
    if(!uri.isValid() || uri.scheme() != QString("californium"))
        return false;
...
QString formatCaliforniumURI(const SendCoinsRecipient &info)
{
    ...
    QString ret = QString("californium:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;
...
bool openCaliforniumConf()
{
    ...
    /* Open californium.conf with the associated application */
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "californium.desktop";
    return GetAutostartDir() / strprintf("californium-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a californium.desktop file to the autostart directory:
