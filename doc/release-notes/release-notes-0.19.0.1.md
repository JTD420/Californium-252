Californium Core version 0.19.0.1 is now available from:
  ...
  <https://californiumcore.org/bin/californium-core-0.19.0.1/>
...
- [Reduce memory](https://github.com/californium/californium/blob/master/doc/reduce-memory.md)
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `californiumd -help` output for these two options for
...
- The `testnet` field in `californium-cli -getinfo` has been renamed to
...
- #15639 californium-wallet tool: Drop libcalifornium_server.a dependency (ryanofsky)
...
- #16366 init: Use InitError for all errors in californiumd/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "californium-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of californiumd build (NicolasDorier)
...
- #16441 remove qt libjpeg check from californium_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_californium to auto generated project files (sipsorcery)
...
- #15352 Reduce noise level in test_californium output (practicalswift)
...
- #15896 feature_filelock, interface_californium_cli: Use PACKAGE_NAME in messages rather than hardcoding Californium Core (luke-jr)
...
- #16277 Suppress output in test_californium for expected errors (gertjaap)
...
- #16489 log: harmonize californiumd logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (californiumVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example californium.conf (fanquake)
...
- #16080 build/doc: update californium_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in californium.conf (fanquake)
...
- #16621 add default californium.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the californium-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, californium.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/californiumd.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/californium/californium/).
