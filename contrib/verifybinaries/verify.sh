...
###   californiumcore.org and californium.org and compares them.
...
WORKINGDIR="/tmp/californium_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://californiumcore.org"
HOST2="https://californium.org"
BASEDIR="/bin/"
VERSIONPREFIX="californium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'californium-',
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "californium.org failed to provide signature file, but californiumcore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "californium.org and californiumcore.org signature files were not equal?"
